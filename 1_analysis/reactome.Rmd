...
#   legend.box.margin = margin (0, -10, 0, -12),
# 
...
reactome_sig[[x]] <- reactome_all[[x]] %>% dplyr::filter(p.adjust <= 0.05) %>% 
  ...
  dplyr::mutate(
    ...
    dplyr::select(c("Count", "GeneRatio", "pvalue", "p.adjust", "logFDR", "qvalue", "Description", "geneID"))
...
for (i in 1:length(fc)) {
  x <- fc[i] %>% as.character()
  react_dot[[x]] <- ggplot(reactome_sig[[x]][1:15, ]) +
  ...
  ggsave(filename = paste0("react_dot_", x, ".svg"), plot = react_dot[[x]], path = here::here("2_plots/reactome/"), 
       width = 88, height = 95, units = "mm")
  
  upset[[x]] <- upsetplot(x = reactome[[x]], 10)
